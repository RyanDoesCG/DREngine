...
vec4 AlbedoForTextureID(vec2 UV, float index)
{
    ...
    if (index == 2.0) return vec4(1.0 - Grid(UV, 0.9));
...
HitPayload IntersectRayAABox(Ray ray, HitPayload last, AABox box)
{
    ...
    if (maxt >= max(0.0, mint) && mint < last.t)
    {
        ...
        vec2 HitUV = vec2(
            0.1 + abs(HitPositionLocalSpace.x * abs(HitNormal.z + HitNormal.y) + HitPositionLocalSpace.z * abs(HitNormal.x)),
            0.1 + abs(HitPositionLocalSpace.y + HitPositionLocalSpace.z * abs(HitNormal.y))) * 0.2;
...
float sphereSDF (vec3 p, vec3 spherePosition, float sphereRadius)
{
    vec3 n = p - spherePosition;
    return length(n) - (sphereRadius);
}

float cubeSDF(vec3 p) {
    vec3 d = abs(p) - vec3(1.0, 1.0, 1.0);
    float id = min(max(d.x, max(d.y, d.z)), 0.0);
...
float together   (float a, float b) { return min(a,  b); }
...
float sceneSDF(vec3 p)
{
   // return cubeSDF(p);
    ...
    float sphere = sphereSDF(p, vec3(0.0, 2.0, 0.0), 1.5);
    float cube = cubeSDF(p - vec3(0.0, 2.0, 0.0));
    float cylinder = cylinderSDF(p - vec3(0.0, 2.0, 0.0), 4.0, 0.5);
    return difference(intersect(sphere, cube), cylinder);
}

vec3 SDFNormal(vec3 p)
{
    return normalize(vec3(
        sceneSDF(vec3(p.x + 0.00001, p.y, p.z)) - sceneSDF(vec3(p.x - 0.00001, p.y, p.z)),
        sceneSDF(vec3(p.x, p.y + 0.00001, p.z)) - sceneSDF(vec3(p.x, p.y - 0.00001, p.z)),
        sceneSDF(vec3(p.x, p.y, p.z + 0.00001)) - sceneSDF(vec3(p.x, p.y, p.z - 0.00001))
    ));
}

#define NUM_STEPS 128
float Closest(Ray ray, float min, float max)
{
    float t = min;
...
HitPayload IntersectRaySDF(Ray ray, HitPayload last)
{
    float t = Closest(ray, 0.0001, 8.0);
    if (t < 8.0 && t < last.t)
    {
        vec4 Colour = vec4(0.2, 0.2, 0.2, 1.0);
        vec4 Material = vec4(1.0, 0.0, 0.0, 2.0);
